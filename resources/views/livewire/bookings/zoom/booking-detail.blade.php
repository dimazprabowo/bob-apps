        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white">Detail Booking Meeting Online</h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Topik</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->topic }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Platform</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->platform }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->start_time->format('H:i') }} - {{ $booking->end_time->format('H:i') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Meeting Link</p>
                        @if($booking->meeting_link)
                            <a href="{{ $booking->meeting_link }}" target="_blank" class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline break-all">{{ $booking->meeting_link }}</a>
                        @else
                            <p class="text-sm text-gray-400">Belum ada link</p>
                        @endif
                    </div>
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div><p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Divisi</p><p class="text-sm text-gray-900 dark:text-white">{{ $booking->booker_divisi ?? '-' }}</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">No. HP</p><p class="text-sm text-gray-900 dark:text-white">{{ $booking->booker_phone ?? '-' }}</p></div>
                    <div><p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Email</p><p class="text-sm text-gray-900 dark:text-white">{{ $booking->booker_email ?? '-' }}</p></div>
            ...
            @if($booking->approver)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div><p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tanggal</p><p class="text-sm text-gray-900 dark:text-white">{{ $booking->approved_at?->format('d M Y H:i') }}</p></div>
                    </div>
                </div>
            @endif
    ...
    <x-approve-booking-modal :show="$showApproveModal">
        <div class="space-y-4">
            <div>
                ...
                <input wire:model="approveMeetingLink" type="url"
