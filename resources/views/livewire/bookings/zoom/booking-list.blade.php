<div>
    <div class="mb-6 flex flex-col md:flex-row md:items-center gap-3">
        ...
        <div class="flex-1 w-full md:w-auto">
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari booking, topik..."
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
        </div>

        <!-- Filter Popover -->
        <x-filter-popover :filters="['statusFilter', 'dateFrom', 'dateTo']">
            <div>
        ...
        <div class="flex items-center gap-2 w-full md:w-auto">
            @can('zoom_bookings_export_excel')
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($bookings as $booking)
                        <tr wire:key="zb-{{ $booking->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <td class="px-4 py-3 whitespace-nowrap">
                                <a href="{{ route('bookings.zoom.show', $booking) }}" wire:navigate class="text-sm font-mono font-semibold text-blue-600 dark:text-blue-400 hover:underline">{{ $booking->booking_code }}</a>
                            </td>
                            <td class="px-4 py-3">
                                <div class="text-sm font-medium text-gray-900 dark:text-white max-w-[200px] truncate">{{ $booking->topic }}</div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $booking->start_time->format('H:i') }} - {{ $booking->end_time->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                <span class="text-sm text-gray-900 dark:text-white">{{ $booking->platform }}</span>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('bookings.zoom.show', $booking) }}" wire:navigate
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Tidak ada data booking meeting online ditemukan</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <div class="inline-block align-bottom w-full bg-white dark:bg-gray-800 rounded-lg text-left shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <form wire:submit="submitBooking">
                        <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">{{ $editMode ? 'Edit Booking Meeting Online' : 'Tambah Booking Meeting Online' }}</h3>

                            <x-zoom-booking-form :editMode="$editMode" :platformOptions="$platformOptions" />
                        </div>
